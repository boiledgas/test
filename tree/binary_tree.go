...
func (t *Binary_tree) Delete(id int) (err error) {
	...
	case node.Left != nil && node.Right != nil:
		{
			...
			new_node, parent := next_node_binary(node)
			// copy
			node.Id = new_node.Id
...
func next_node_binary(node *binary_node) (next *binary_node, parent *binary_node) {
	parent = node
	next = node.Right
